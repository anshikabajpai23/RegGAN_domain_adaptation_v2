...
| Localize/crop | **Tried — see step 13** | Targets in-plane detail = 0.6% of real-PD error. Alone: +0.0130, below floor. Combined with sequence (step 15): part of the first confirmed real gain. |
...
| Slice-sequence architecture (ConvLSTM / cross-slice attention) instead of fixed 2.5D stack (Anshika) | **Tried — see steps 14, 15** | Check 2 showed the model ignores neighbors at FIXED channel positions (B≈A, C hurts 17/17). A sequence model that learns to weight/aggregate neighboring slices adaptively, rather than baking z-context into 3 fixed input channels, targets the same failure (no usable z-context) via a different mechanism than P4's stride fix or 3D nnU-Net's full-volume patch. Alone: +0.0178, below floor. Combined with crop (step 15): +0.0277, above floor — first confirmed real gain besides P1. |
...
| 6 | k-fold (5 folds) | 5×4 h | CI + checkpoint | **Done, read on the 17pt test set (`analysis_phase4_17patients.ipynb`).** kfold0=0.7780, kfold1=0.7723, kfold2=0.7769, kfold3=0.7827, kfold4=0.7783. Mean=0.7776, range=0.0104 (kfold1 to kfold3), **all 5 within the 0.0226 noise floor of replica_seed42 (0.7780)**. Which 3 real-PD patients get held out barely moves 17pt test Dice — the fixed 13/2 split isn't accidentally lucky or unlucky. Range (0.0104) is smaller than the seed42-vs-seed7 per-patient floor (0.0226), suggesting seed noise, not the train/val split choice, is the larger source of run-to-run variance. `scripts/aggregate_kfold_results.py` (per-patient 15-pt CV table) still pending — this is the secondary test-set-generalization read only. |
...
| 13 | **Crop/localize** fixed bbox, one retrain | prep + 4 h | targets in-plane detail (0.6% of core error) — expected small | **Done, confirmed on 17pt test set.** 0.7780→0.7910, Δ=+0.0130 — largest single-change gain seen so far, but still below the 0.0226 noise floor. Not confirmed real by the same strict standard applied to P4/P2/P1, but the biggest of the "no" verdicts so far and worth a second read (repeat seed or TTA) before ruling out. |
| 14 | **Slice-sequence architecture** (ConvLSTM, Anshika's candidate) | build + 4 h | adaptive z-context aggregation vs P4's fixed-channel stack | **Done, confirmed on 17pt test set.** 0.7780→0.7958, Δ=+0.0178 — largest gain of any tested change, above P4's Δ=+0.0076 and crop's Δ=+0.0130, but still below the strict 0.0226 noise floor. Same caveat as crop: not yet "confirmed real" by the standard used to reject P4/P2, but the most promising single lever tested to date — candidate for a repeat-seed run to see if the gain replicates before investing further (e.g. wider T, cross-slice attention instead of ConvLSTM). |
| 15 | **Crop + Sequence combined** — sequence architecture retrained on the crop-prepped data | build + 6–7 h | do crop's localization and sequence's z-context aggregation stack, or overlap | **Done, confirmed on 17pt test set — first non-P1 result to clear the noise floor.** 0.7780→0.8057, **Δ=+0.0277 — above the 0.0226 floor.** Roughly additive: crop alone +0.0130, sequence alone +0.0178 (sum ≈ +0.0307), combined lands close to but slightly under that sum, consistent with the two gains being largely independent rather than the same effect counted twice. 2-val-patient read agrees in direction: dice_real_va 0.7809→0.8067, both val patients moved up together (0.7773/0.7968 vs 0.7545/0.7506) — same-direction movement, not the noise draw's push-pull pattern. **Coverage caveat CLOSED 2026-09-22.** `crop_box.json` was computed from the 13 real-PD TRAIN patients only, so it had to be checked against the 17 TEST patients' actual GT extent — any test-patient meniscus outside the box would be a guaranteed miss baked into the geometry, not a model failure. `scripts/check_crop_box_coverage.py` has now been run against the pulled box (rows [79,373), cols [29,230), 294×201, +15px pad): **100.00% of GT voxels inside the box for all 17/17 patients, 0 voxels outside, 0 fully-missed slices.** The box generalizes from train to test without loss, so **0.8057 is not distorted by box geometry** and stands as a real result. (It is a generous box, ~2.5× area magnification, not a tight localization — so the gain is not coming from extreme zoom either.) |

Realistic: P4 + P2 + P1 + free wins → **0.78 → 0.81–0.83**, capped by the ceiling from step 2. Crop+sequence's confirmed +0.0277 already exceeds the low end of this range, pending the box-coverage caveat above.

**17pt full comparative pass (this notebook run):** ranked by mean 17pt Dice — **crop+sequence (0.8057)** > sequence (0.7958) > crop (0.7910) > P4 (0.7856) > kfold3 (0.7827) > P2 (0.7795) > kfold4 (0.7783) ≈ replica/replica_v2/kfold0 (0.7780) > kfold2 (0.7769) > kfold1 (0.7723) > P1 (0.7489). `replica_seed42_v2` (re-pulled, re-inferred) landed at exactly 0.7780, Δ=0.0000 vs the original pull — confirms the inference pipeline is fully deterministic and reproducible, as expected (no `--tta`, eval mode). Applying the SAME strict 0.0226 threshold used to reject P4 and P2, crop and sequence individually still don't clear it — but **crop+sequence combined does** (Δ=+0.0277), making it the first Phase 4 change other than P1 to be a confirmed real effect, pending the box-coverage check above.
