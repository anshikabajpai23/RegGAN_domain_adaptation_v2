"""
finetune_meniscus_v7_p1.py
===========================
Phase 4, step 9 (P1): sampler cap on real PD.

Tests whether the ~21.6x/epoch real-PD repetition (check 3) is limiting val
Dice. Byte-for-byte identical to finetune_meniscus_v7_replica.py — same
loss, optimizer, seed default (42), fake/real data sources, val-dump/history
logic — EXCEPT for the ONE change docs/reggan_dice_debug.md §4.9 specifies,
inside run_epoch:

  Original: every fake batch pulls a real batch from a cycling iterator that
  refills on StopIteration -- ~1189 fake batches/epoch, ~55 real batches
  available, so the small real set is repeated ~21.6x every epoch.

  P1: during TRAINING only, a real batch is drawn only every `real_stride`
  fake batches (real_stride = ceil(n_fake_batches / n_real_batches), computed
  at runtime from actual loader lengths -- ~22, matching check 3's number).
  Once the real set is exhausted for the epoch, NO wraparound -- no more real
  gradient contributes for the rest of that epoch. Net effect: each real
  slice is seen close to once per epoch, spread across it, instead of ~22x
  concentrated by however DataLoader shuffling orders it.

  On fake-batch iterations where no real batch is drawn, the step uses
  loss_f alone (still a real gradient step -- fake data justifies its own
  update regardless of whether a real batch happens that iteration).

  VALIDATION is UNCHANGED -- real_stride is not applied when train=False, so
  vd/dice_real_va is computed exactly as the replica does (full real_val
  pass every epoch, unaffected by the training-time cap). This keeps the
  metric you're judging the experiment by identical to every other Phase 4
  run's metric.

Per the doc's own caveat: "early stopping may trigger later since real
signal per epoch is 22x smaller" -- run with --epochs 100 (not 50) and the
same --patience 10.

One accounting change from the replica, made necessary by the above (not a
free choice): `dice_real_tr` in run_epoch's return / history.csv is now
averaged over the number of iterations that ACTUALLY had a real batch
(n_real_dice), not over every fake iteration (n) -- averaging over n would
silently dilute the metric with iterations that had no real prediction at
all. dice_fake_tr is unaffected (still averaged over all n, since fake data
appears every iteration, capped or not).

Usage: identical CLI to finetune_meniscus_v7_replica.py (same
--fake_data_root / segmentation_data_v2, same --real_data_root), no new
required args -- real_stride is computed automatically from the loaders.
"""
import argparse
import csv
import logging
...
def run_epoch(model, real_loader, fake_loader, optimizer,
              merged_loss_fn, fake_loss_fn, device, train=True, real_stride=None):
    """
    real_stride: only applied when train=True. None (or eval mode) reproduces
    the replica's original cycling-with-wraparound behavior EXACTLY. An int
    means: pull a fresh real batch only every `real_stride` fake iterations,
    and do NOT refill on exhaustion within the epoch (no wraparound).
    """
    model.train() if train else model.eval()
    total_loss, dice_sum, dice_fake_sum, n, n_real = 0.0, 0.0, 0.0, 0, 0
    real_iter = iter(real_loader)
    real_exhausted = False
    capped = train and real_stride is not None

    with torch.set_grad_enabled(train):
        for fake_idx, fake_batch in enumerate(fake_loader):
            xf, yf = fake_batch["image"].to(device), fake_batch["mask"].to(device)
            logits_f = model(xf)
            loss_f   = fake_loss_fn(logits_f, yf)

            pull_real = (not capped) or (fake_idx % real_stride == 0 and not real_exhausted)
            ...
            loss = loss_f + loss_r if loss_r is not None else loss_f
            if train:
    ...
    return (total_loss    / max(n, 1),
            dice_sum      / max(n_real, 1),   # averaged over real-bearing iters only
...
def main():
    ...
    ap.add_argument("--fake_data_root",   required=True,
                    help="segmentation_data_v2 — same data source as the replica, UNCHANGED")
    ap.add_argument("--real_data_root",   required=True)
    ap.add_argument("--out_dir",          required=True)
    ap.add_argument("--epochs",           type=int,   default=100,
                    help="doc explicitly asks for 100 here, not the replica's 50, "
                         "since capped real exposure may delay the val plateau")
    ...
    n_fake_batches = len(fake_train_loader)
    n_real_batches = len(real_train_loader)
    real_stride = math.ceil(n_fake_batches / n_real_batches)
    log.info(f"P1: real_stride={real_stride}  ({n_fake_batches} fake batches / "
             f"{n_real_batches} real batches per epoch -> "
             f"real seen ~once/epoch instead of ~{n_fake_batches/n_real_batches:.1f}x)")
    ...
    for epoch in range(args.epochs):
        ...
        tl, td, tdf = run_epoch(model, real_train_loader, fake_train_loader,
                                optimizer, merged_loss_fn, fake_loss_fn, device,
                                train=True, real_stride=real_stride)
        vl, vd, vdf = run_epoch(model, real_val_loader,   fake_val_loader,
                                optimizer, merged_loss_fn, fake_loss_fn, device,
                                train=False, real_stride=None)   # val UNCAPPED, unchanged
        scheduler.step()
    ...
    log.info("Compare train/val gap and epoch-of-plateau vs replica's gap=+0.144 @ ep10.")
