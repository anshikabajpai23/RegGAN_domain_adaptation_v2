"""
finetune_meniscus_v12_photometric.py
=====================================
v12_photometric: Same as v7_mixed (DESS baseline, 15 real PD + fake PD)
                 but adds photometric augmentation on the fake PD branch.

Augmentations added to fake PD (not real PD — real PD already has its own aug):
  - Gamma correction:     U(0.7, 1.5)
  - Gaussian blur:        σ ~ U(0.3, 1.2), 50% chance
  - Contrast stretch:     random 2nd/98th percentile rescale, 50% chance

Key difference from v3_photometric (Dice=0.681):
  v3 used fake PD only (no real PD labels) and ran from baseline.
  v12 uses v7_mixed recipe — 15pt real PD + fake PD together.
  Fair ablation: does photometric aug on fake PD help when paired with real PD?
...
def photometric_augment(img: torch.Tensor) -> torch.Tensor:
    """Photometric augmentation on a (C,H,W) tensor in [-1,1].
    Applies gamma correction, optional Gaussian blur, optional contrast stretch.
    Operations preserve the [-1,1] range.
    """
    # Shift to [0,1] for photometric ops
    x = (img + 1.0) / 2.0
    x = x.numpy()
...
class PhotometricFakeDataset(torch.utils.data.Dataset):
    """Wraps Meniscus2_5DDataset and applies photometric aug to images."""
    def __init__(self, base_dataset: Meniscus2_5DDataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.base.augment:
            sample["image"] = photometric_augment(sample["image"])
        return sample
...
class SoftDiceLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        for c in range(1, self.n_classes):
            ...
            tp = (p * t).sum()
            loss += 1.0 - (2*tp + self.eps) / (p.sum() + t.sum() + self.eps)
...
def main():
    ...
    log.info(f"Device: {device}  |  LR: {args.lr}  |  Photometric aug on fake PD  |  Data: mixed real+fake PD (DESS base)")

    model = build_model(args.pretrained_ckpt, device)

    fake_base_train = Meniscus2_5DDataset(
        os.path.join(args.fake_data_root, "train", "images"),
        os.path.join(args.fake_data_root, "train", "masks"), augment=True)
    fake_base_val   = Meniscus2_5DDataset(
    ...
    fake_train = PhotometricFakeDataset(fake_base_train)
    fake_val   = PhotometricFakeDataset(fake_base_val)
    ...
    fake_loss_fn = lambda logits, y: (nn.CrossEntropyLoss(weight=weights)(logits, y) + dice_loss(logits, y))
