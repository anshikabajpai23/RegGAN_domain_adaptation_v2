"""
finetune_meniscus_v7_crop_seq_g2.py
====================================
Phase 5, G2 — region-darker-than-rim loss, on the crop+sequence base.
...
THE ONE CHANGE: `+ lam * rim_loss` on BOTH branches. The prior needs no labels,
so it applies to fake and real alike.

THE PRIOR WAS MEASURED FIRST (doc §5.2 gate, precheck_rim_darkness.py):
    real PD 1.000 of slices, fake PD 0.964 -- both far above the 0.80 threshold.
    fake median gap 0.1171 -> suggested margin 0.0293 (25% of it).
The conservative (smaller) margin is the default: too large a margin pushes the
model to over-darken, which shows up as rising false negatives (doc §5.5).

LAMBDA IS CALIBRATED, NOT GUESSED. The doc requires the rim term to be 5-10% of
the base loss at epoch 1. A hardcoded lam would be wrong at this LR/scale, so
this script measures both losses over the first --calib_batches and solves
    lam = target_frac * base_loss / rim_loss
before training starts. Pass --lam explicitly to override.

WATCH rim_tr / rim_frac IN history.csv. If rim_frac drifts far above ~0.10 the
term is dominating; if rim_tr goes to 0 immediately the prior was already
satisfied and the run is a no-op vs the baseline.
"""
import argparse
import csv
import json
...
from rim_loss import rim_loss, meniscus_prob
...
CENTRE = 2          # centre of the 5-slice window
...
def calibrate_lambda(model, real_loader, fake_loader, merged_loss_fn, fake_loss_fn,
    ...
    """doc §5.3: pick lam so the rim term is target_frac of the base loss.
    Hardcoding lam would be wrong at this scale, so it is measured."""
    model.eval()
    real_iter = iter(real_loader)
    ...
    if n == 0 or rim_tot <= 1e-9:
        log.warning("Rim loss is ~0 at init: the prior is already satisfied everywhere, "
                    "so G2 will be close to a no-op. Falling back to lam=1.0.")
...
def main():
    ...
    ap.add_argument("--margin", type=float, default=0.0293,
                    help="normalised intensity; default = 25%% of the measured fake-PD gap")
    ap.add_argument("--rim_k", type=int, default=5, help="rim width in px")
    ...
    ap.add_argument("--lam_target_frac", type=float, default=0.07,
                    help="rim term as a fraction of base loss at init (doc says 0.05-0.10)")
    ...
    log.info(f"Device: {device}  |  LR: {args.lr} -> 1e-8  |  G2: rim loss on crop+sequence base")
    log.info(f"  margin={args.margin} (normalised intensity)  rim_k={args.rim_k} px")
    ...
    fake_train = SequenceMeniscusDataset(os.path.join(args.fake_data_root, "train", "images"),
                                         os.path.join(args.fake_data_root, "train", "masks"), augment=True)
    fake_val = SequenceMeniscusDataset(os.path.join(args.fake_data_root, "val", "images"),
                                       os.path.join(args.fake_data_root, "val", "masks"), augment=False)
    real_train = SequenceRealPDDataset(os.path.join(args.real_data_root, "train", "images"),
                                       os.path.join(args.real_data_root, "train", "masks"), augment=True)
    real_val = SequenceRealPDDataset(os.path.join(args.real_data_root, "val", "images"),
    ...
    # replica schema + rim_tr/rim_va/rim_frac appended; trailing extras are ignored
    # by the notebook's Section 2/3 readers
