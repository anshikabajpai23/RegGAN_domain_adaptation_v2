"""
infer_real_pd_v3_pos.py
=========================
Inference for the Phase 5 E1 checkpoint on the PLAIN 2.5D architecture
(run_2_5d_e1_seed42) -- position channel, NO crop, NO sequence. Identical
predict/save round trip to infer_real_pd_v3.py, plus the single-scalar
position value each sample needs (model_2_5d_pos.Unet2_5DPos.forward(x, pos)
takes ONE pos per sample -- the centre slice's own position -- unlike the
crop+sequence E1 model, which needs one pos per timestep).

Position, same formula as training (finetune_meniscus_v7_replica_e1.py):
    pos = 2 * |idx/(n-1) - 0.5|,  0 at the volume centre (notch), 1 at outer edges

NO LOOKUP TABLE NEEDED, same reasoning as the crop+sequence E1 inference
...
Usage:
    python segmentation/infer_real_pd_v3_pos.py \
        --pd_root   /N/.../pd-files \
        --filenames AC0D5A4D78B628_SAG_PD_TSE_6.nii.gz ... \
        --ckpt      /N/.../segmentation_runs/run_2_5d_e1_seed42/ckpt_best.pth \
        --out_dir   /N/.../results/real_pd_predictions_2_5d_e1_17pt
"""
import argparse
import logging
import os
...
from model_2_5d_pos import load_trained_2_5d_pos_model
...
def predict_volume_pos(vol, model, device, batch_size=8, tta=False):
...
def main():
    ...
    model = load_trained_2_5d_pos_model(args.ckpt, device)
    model.eval()
    log.info(f"Loaded E1 (position-channel, plain 2.5D) checkpoint from {args.ckpt}")
    ...
    for fname in args.filenames:
        ...
        preds = predict_volume_pos(vol, model, device, args.batch_size, tta=args.tta)
