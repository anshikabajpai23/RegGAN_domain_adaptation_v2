"""
finetune_meniscus_v4_mixed.py
==============================
Fine-tune run_002_v2 on MIXED data: 10 real PD (binary masks) + fake PD
(segmentation_data_v2, 3-class masks). Directly targets the domain gap.

Per epoch:
  - Real PD batches: MergedLoss (binary CE + binary Dice, classes 1+2 merged)
  - Fake PD batches: standard v2 loss (weighted CE + SoftDice, 3-class)
  Real PD is oversampled each epoch to match fake PD batch count.
...
from dataset_2_5d_v2    import Meniscus2_5DDataset   # fake PD, 3-class
from dataset_2_5d_realpd import RealPDDataset          # real PD, binary
...
def run_epoch(model, real_loader, fake_loader, optimizer,
    ...
    with torch.set_grad_enabled(train):
        for fake_batch in fake_loader:
            ...
            # Dice on merged meniscus for real PD val tracking
...
def main():
    ...
    ap.add_argument("--fake_data_root",   required=True,
                    help="segmentation_data_v2/ directory")
    ...
    log.info(f"Device: {device}  |  LR: {args.lr} → 1e-8  |  Data: mixed real+fake PD")
