"""
finetune_meniscus_v7_p4.py
===========================
Phase 4, step 7 (P4): fake-PD 2.5D stride 4-5 + equalized augmentation.

Byte-for-byte identical to finetune_meniscus_v7_replica.py — same loss
(MergedLoss w_bg=0.1, SoftDice, weighted CE), same optimizer, same seed
default (42), same val-dump/history logic — EXCEPT for exactly the three
things docs/reggan_dice_debug.md §4.7 specifies:

  1. fake_train/fake_val 2.5D stack uses stride_choices=(4,5) instead of the
     fixed (1,) — neighbors drawn per-sample at 4 or 5 DESS-slice spacing
     (3.2-4.0mm), bracketing real PD's 3.6mm, instead of the original 0.8mm.
     edge_fill="nearest" so volume-boundary handling matches
     infer_real_pd_v3.py's clamp instead of the old "always fall back to
     center" rule (see dataset_2_5d_v2.py's docstring for why this is safe
     ONLY on data with no interior gaps, which --fake_index_file's source
     data must satisfy: segmentation_data_v3/, prepared with
     --min_meniscus_pixels 0, NOT segmentation_data_v2/).
  2. fake_train is restricted via --fake_index_file to train_index_posonly.txt
     (mask-bearing slices only) — same POPULATION as v2's train split, just
     drawn from the v3 (no-gaps) directory so "nearest" edge-fill is valid.
  3. real_train/real_val use RealPDDataset(aug_mode="equalized") — brightness/
     noise augmentation now matches the fake branch exactly (multiplicative
     U(0.8,1.2) p=1.0, noise sigma=0.02 p=1.0, clipped after each op) instead
     of the original additive/lower-probability version.

Everything else — including run_epoch, the losses, eval_real_per_patient,
dump_val_predictions, the training loop, checkpoint selection on `vd` — is
copied unchanged from the replica so this run is comparable to
run_v7_replica_seed42 / run_v7_replica_seed7 with exactly these three things
different ("one change per retrain" bundled as specified, not three separate
retrains).

After training, run scripts/phase4_3_edge_core_split.py on this run's
val_predictions/per_slice_dice.csv for the edge/core split the doc asks for.

Usage: identical CLI to finetune_meniscus_v7_replica.py, plus:
  --fake_index_file   path to train_index_posonly.txt (required)
  --stride_choices     e.g. "4 5" (default: 4 5)
  --aug_mode           "equalized" (default) or "original" (to isolate the
                        stride change alone, if you want a fourth run)
"""
import argparse
import csv
import logging
...
def main():
    ...
    ap.add_argument("--fake_data_root",   required=True,
                    help="segmentation_data_v3 root (NOT v2 — must have no interior gaps)")
    ap.add_argument("--fake_index_file",  required=True,
                    help="train_index_posonly.txt from build_v3_index_files.py")
    ...
    ap.add_argument("--stride_choices",   type=int, nargs="+", default=[4, 5],
                    help="P4's core change: neighbor offset k in [i-k,i,i+k], drawn per sample")
    ap.add_argument("--aug_mode",         default="equalized", choices=["equalized", "original"],
                    help="real-branch augmentation; 'equalized' matches the fake branch (P4 default)")
    args = ap.parse_args()
    ...
    log.info(f"P4: stride_choices={tuple(args.stride_choices)}  aug_mode={args.aug_mode}  "
             f"fake_index_file={args.fake_index_file}")
    ...
    fake_train = Meniscus2_5DDataset(
        ...
        stride_choices=tuple(args.stride_choices), edge_fill="nearest",
        index_file=args.fake_index_file)
    ...
    log.info(f"Fake PD — train: {fake_train_scans} scans, {len(fake_train)} slices (posonly index)  |  "
    ...
    log.info("Next: run scripts/phase4_3_edge_core_split.py on val_predictions/per_slice_dice.csv "
             "for the edge/core split vs the replica.")
