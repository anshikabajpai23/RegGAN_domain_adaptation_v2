"""
finetune_meniscus_v2.py
========================
Improved fine-tuning over v1:
  1. Data augmentation (flips, brightness jitter, noise) via dataset_2_5d_v2
  2. Class-weighted CrossEntropy (downweight background, upweight meniscus)
  3. Cosine LR annealing (instead of fixed LR)
"""
import argparse
import logging
import os
...
def main():
    ...
    # Class weights: background=0.1, lateral=1.5, medial=1.5
    ...
    # Cosine annealing: LR decays from lr → eta_min over all epochs
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=args.epochs, eta_min=1e-7
    )

    # Class-weighted CE: downweight background, upweight meniscus classes
    ...
    log.info(f"Class weights: bg={args.class_weights[0]} lat={args.class_weights[1]} med={args.class_weights[2]}")
    log.info(f"LR schedule: CosineAnnealing lr={args.lr} → 1e-7 over {args.epochs} epochs")
