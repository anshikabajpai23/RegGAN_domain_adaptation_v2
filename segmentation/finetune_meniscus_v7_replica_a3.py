"""
finetune_meniscus_v7_replica_a3.py
====================================
Phase 5, A3 — slice-position weighted loss, on the PLAIN 2.5D replica base
(independent of crop+sequence, so this is the clean one-change-vs-replica
comparison, exactly like P1/P2/P4 in Phase 4 -- not a change stacked on top
of two other already-combined changes).
...
scripts/build_slice_weights.py, run against the UNCROPPED data this time
(crop only affects in-plane pixels, not which slice is a span end, but the
weights JSON must be built from the same data root this script reads, or the
patient/slice keys used to look up a weight will not match).

    --variant up    end slices count up to 2x
    --variant down  end slices count as 0.3x  (Check 4A's predicted winner)
    --variant none  all weights 1.0  (CONTROL -- isolates the per-sample vs.
                     per-batch loss reduction change on its own; see the
                     crop+sequence A3 script's docstring for why this control
                     exists and why it measured negligible in practice)

Compare dice_real_va against run_v7_replica_seed42 (0.7809 val, 0.7780 test),
using the 0.0226 noise floor -- NOT against crop+sequence.
"""
import argparse
import csv
import logging
...
def main():
    ...
    log.info(f"Device: {device}  |  LR: {args.lr} -> 1e-8  |  A3 variant: {args.variant}  |  PLAIN 2.5D, uncropped")
