"""
finetune_meniscus_v7_replica_e1.py
=====================================
Phase 5, E1 — slice position as a 4th input channel, on the PLAIN 2.5D
replica base (independent of crop+sequence). Same rationale as the
crop+sequence version, cleaner comparison: this diffs directly against
run_v7_replica_seed42, exactly like P1/P2/P4 did.

BASE: everything from finetune_meniscus_v7_replica.py is copied verbatim
except build_model() (swapped for model_2_5d_pos.build_2_5d_pos_model, the
zero-init 4-channel wrapper -- proven bit-identical to plain smp.Unet at step
0, max diff 0.000e+00) and the two dataset classes (thin subclasses adding a
...
from model_2_5d_pos import build_2_5d_pos_model, verify_zero_init
...
def main():
    ...
    log.info(f"Device: {device}  |  LR: {args.lr} -> 1e-8  |  E1: position channel, PLAIN 2.5D, uncropped")
    ...
    model = build_2_5d_pos_model(args.pretrained_ckpt, device,
                                 pretrained_n_classes=PRETRAINED_N_CLASSES, n_classes=N_CLASSES)
    log.info(f"Loaded DESS baseline from {args.pretrained_ckpt}, wrapped with Unet2_5DPos")
