"""
finetune_meniscus_v3_resnet50.py
=================================
Same as finetune_meniscus_v2.py but uses ResNet50 encoder instead of ResNet34.

Change vs v2:
  + encoder_name="resnet50"  (deeper, more capacity)
  Dataset, loss, LR: identical to v2.

Purpose: test whether stronger encoder alone improves real PD Dice.
Baseline to beat: run_002_v2 Dice=0.6854 (ResNet34).
"""
import argparse
import logging
import os
...
def build_model(pretrained_ckpt=None, device="cpu"):
    # pretrained_ckpt is ResNet34 — incompatible architecture, cannot transfer weights.
    # Use ImageNet pretrained ResNet50 encoder; head is randomly initialised for N_CLASSES.
    model = smp.Unet(
    ...
    if pretrained_ckpt:
        log.info("ResNet50 encoder: using ImageNet pretrained weights (ResNet34 ckpt not transferable)")
    ...
    log.info("Encoder: ResNet50  |  Head: 5 → 3 classes")
...
def main():
    ...
    log.info(f"Device: {device}  |  Encoder: ResNet50  |  Baseline to beat: 0.6854 (ResNet34)")
