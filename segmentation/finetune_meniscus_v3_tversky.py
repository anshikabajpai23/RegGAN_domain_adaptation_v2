"""
finetune_meniscus_v3_tversky.py
================================
Same as finetune_meniscus_v2.py but replaces SoftDice with TverskyLoss.

Tversky index:  T = TP / (TP + alpha*FP + beta*FN)
  alpha=0.3, beta=0.7  → penalises false negatives more than false positives
  → recovers more meniscus voxels (better recall on small structures)

Change vs v2:
  + TverskyLoss(alpha=0.3, beta=0.7) replacing SoftDiceLoss
  Dataset: dataset_2_5d_v2 (same as v2, no rotation)
  LR schedule, class weights: identical to v2.

Purpose: test whether Tversky loss alone improves real PD Dice.
"""
import argparse
import logging
import os
...
from dataset_2_5d_v2 import Meniscus2_5DDataset  # same dataset as v2
...
class TverskyLoss(nn.Module):
    ...
    alpha < beta means FN penalised more → better recall on small structures.
    """
...
def main():
    ...
    ap.add_argument("--tversky_alpha", type=float, default=0.3,
                    help="FP weight — lower = tolerate more false positives")
    ap.add_argument("--tversky_beta",  type=float, default=0.7,
                    help="FN weight — higher = penalise false negatives more")
    args = ap.parse_args()

    os.makedirs(args.out_dir, exist_ok=True)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    log.info(f"Using device: {device}")
    log.info(f"Loss: weighted CE + TverskyLoss(alpha={args.tversky_alpha}, beta={args.tversky_beta})")
    log.info("Augmentation: hflip + vflip + brightness + noise  [v2 dataset, no rotation]")
    ...
    tversky_loss = TverskyLoss(n_classes=N_CLASSES,
                               alpha=args.tversky_alpha,
                               beta=args.tversky_beta)

    def loss_fn(logits, targets):
        return ce_loss(logits, targets) + tversky_loss(logits, targets)
