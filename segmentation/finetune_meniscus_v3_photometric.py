"""
finetune_meniscus_v3_photometric.py
=====================================
Same as finetune_meniscus_v2.py but uses dataset_2_5d_v3_photometric which adds
heavy photometric augmentation: gamma, Gaussian blur, contrast stretch.

Change vs v2:
  + gamma correction (0.7–1.5)
  + Gaussian blur (sigma 0.3–1.2)
  + contrast stretch (random black/white point shift)
  Loss, LR, encoder: identical to v2.

Purpose: test whether photometric aug bridging fake→real PD domain improves Dice.
...
from dataset_2_5d_v3_photometric import Meniscus2_5DDataset
...
def build_model(pretrained_ckpt=None, device="cpu"):
    ...
    if pretrained_ckpt:
        ...
        log.info(f"Loaded from {pretrained_ckpt}")
    old_head = model.segmentation_head[0]
    model.segmentation_head[0] = nn.Conv2d(old_head.in_channels, N_CLASSES,
...
def run_epoch(model, loader, optimizer, loss_fn, device, train=True):
    ...
    with torch.set_grad_enabled(train):
        for batch in loader:
            ...
            for c, d in enumerate(dice_per_class(preds.argmax(1), y)): dice_sums[c] += d
            n += 1
...
def main():
    ...
    log.info(f"Device: {device}  |  Augmentation: v2 + gamma + blur + contrast  |  Baseline: 0.6854")
